...
fn walk_b<'a>(current: &'a FileTreeNode<'a>, space_needed_to_free: usize) -> Option<&'a FileTreeNode<'a>> {
    // if we aren't a directory return None
    // else check if any of our children are directories, and if any of them are larger than space_needed_to_free
    // if so, recurse on each of these child directories and return the smallest result
    // otherwise, check if we are larger than space_needed_to_free
    // if we are, return ourselves
    // otherwise, return None

    match &current.children {
        None => None,
        Some(children) => {
           let smallest_qualifying_child = children
               .values()
               .filter(|entry| entry.children.is_some())
               .filter(|entry| entry.size >= space_needed_to_free)
               .map(|entry| walk_b(entry, space_needed_to_free))
               .filter_map(|entry| entry)
               .min_by_key(|entry| entry.size);

            match smallest_qualifying_child {
                Some(entry) => Some(entry),
                None => if current.size >= space_needed_to_free {
                    Some(current)
                } else {
                    None
                }
